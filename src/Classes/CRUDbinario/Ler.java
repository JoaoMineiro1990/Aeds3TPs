...
public class Ler {
    ...
    public static void lerPokemonPorNome(String caminhoArquivo, String nomeProcurado) {
        try (RandomAccessFile raf = new RandomAccessFile(caminhoArquivo, "r")) {
            raf.readInt(); // pula o último ID global
            ...
            while (raf.getFilePointer() < raf.length()) {
                ...
                if (name.equalsIgnoreCase(nomeProcurado)) {
                    System.out.println("--- POKÉMON ENCONTRADO ---");
                    System.out.println("ID: " + id);
                    System.out.println("Cova: " + cova);
                    System.out.println("TamanhoBytes: " + tamanhoBytes);
                ...
                // pula para o próximo Pokémon
                raf.seek(posAntesDoId + 4 + 1 + 4 + tamanhoBytes);
            }

            System.out.println("❌ Pokémon com nome \"" + nomeProcurado + "\" não encontrado.");
        } catch (IOException e) {
            throw new RuntimeException("Erro ao ler Pokémon do arquivo", e);
        }
    }
